...
```{r,message=FALSE}
ggplot(data = contributions2, aes(x = entity_type, y =transaction_amt/1000000, fill = entity_type))+
  labs(title = "Transaction Amount in 2012 for Different Entity Type")+
  guides(color = guide_legend("entity type")) +
...
contributions2 %>%
  filter(transaction_type =="24A",transaction_type =="24E", entity_type == "CCM")
glimpse(contributions2)
ggplot(data = contributions2,aes(x = transaction_type, y = transaction_amt/1000000, fill = transaction_type))+
  geom_bar(stat = "identity")

```

plot<- function(data=data1,entity)
lapply(contributions2$entity_type, FUN = plot)
